...
var createReducer = function () {
    ...
    return function (reducer, initialState, initializer) {
        ...
        var ref = hooks_1.useRef(initializer(initialState));
        var _a = hooks_1.useState(ref.current), setState = _a[1];
        var dispatch = hooks_1.useCallback(function (action) {
            ref.current = reducer(ref.current, action);
            setState(ref.current);
            return action;
        }, [reducer]);
        var dispatchRef = hooks_1.useRef(composedMiddleware({
